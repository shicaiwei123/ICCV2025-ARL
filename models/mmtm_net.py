...
class MMTM_Net(nn.Module):
    ...
    def forward(self, img_rgb, img_depth):
        ...
        if self.pe:
            ...
            x_depth, mu_depth, std_depth = self.depth_p(x_depth)
            ...
            out_rgb = self.fc(torch.cat((x_rgb, torch.zeros_like(x_depth)), dim=1))
            out_depth = self.fc(torch.cat((torch.zeros_like(x_rgb), x_depth), dim=1))

            return out, mu_rgb, std_rgb, mu_depth, std_depth, out_rgb, out_depth
        else:
