...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,args,modality, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
    ...
    def forward(self, x):
        ...
        # patch_score = torch.zeros(x_spp.shape[0], 100, feature_num)
        #
        ...
        #
        #     patch_logits = self.head(patch_feature_flatten)
        #     patch_score[:, :, i] = patch_logits

        # x = self.head(x)
        if self.modality=='visual':
            if self.args.current_epoch > self.args.start:
                weight = torch.tensor(self.args.visual_weight, requires_grad=False)
                gamma = torch.tensor(self.args.gamma, requires_grad=False)
                out = GradScale.apply(x, weight, gamma)
            else:
